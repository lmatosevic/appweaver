...
export interface EvictionIndex {
  /**
   * Adds a key-value pair to the cache with associated metadata.
   *
  ...
  /**
   * Removes an entry from the collection based on the specified key.
   *
   * @param {string} key - The unique identifier for the entry to be removed.
   */
   ...
   *
   * @param {string} key - The unique identifier for the cache entry to update.
  ...
  /**
   * Removes and returns a list of candidates that meet the eviction criteria.
   *
   ...
   */
  evictCandidates(count: number, now: number, gracePeriod: number): string[];
}
