...
Per model, the type file holds `<Model>`, `<Model>Single`, `<Model>Multiple`, `<Model>RelationOutput` (the shape of
the model nested in another model's response), `<Model>Create`, `<Model>Update`, `<Model>RelationCreate`,
`<Model>RelationUpdate`, `<Model>RelationInput`, `<Model>Query`, `<Model>Sort`, `<Model>Aggregate`, and last
`<Model>ResourceService`, the `injectService` type of that model.
